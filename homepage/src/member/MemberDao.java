...
public class MemberDao {
	...
	public boolean update(MemberDto dto) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append(
				" update member set tel = ?, email = ?, zipcode = ?, address1 = ?, address2 = ?, job = ? where id = ? ");
		try {
			pstmt = con.prepareStatement(sql.toString());
	...
	public List<MemberDto> list(Map map) {
		...
		sql.append(" 		ORDER BY mdate desc    "); // 최근가입자가 위에올라옴
		sql.append(" 	) ");
	...
	public boolean duplicateId(String id) {
		...
		ResultSet rs = null;
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT COUNT(id) cnt FROM member WHERE id=? ");
