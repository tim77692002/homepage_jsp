...
CREATE TABLE bbs ( 
  ...
  passwd  VARCHAR(15)   NOT NULL,  -- 비밀 번호 (수정 삭제시 사용) (로그인 기능 있을때는 필요 없음)
...
select * from bbs order by grpno desc, ansnum; 

--filename 컬럼 추가
alter table bbs add(filename varchar(50) null, filesize number default 0);


--부모글 번호를 저장할 컬럼 추가(답변이 있는 부모글 삭제시 확인)
alter table bbs add(refno number default 0)

select bbsno, grpno, indent, ansnum, refno from bbs order by grpno desc, ansnum 
...
- NVL(): null 값을 0으로 변경 가능함. 오라클에서 사용
...
--total
select count(*) from bbs where wname like '%왕눈%' 
...
UPDATE bbs SET wname='왕눈이', title='비오는날', content='개구리 연못' WHERE bbsno = 1; 
...
-- 패스워드 검사 
SELECT COUNT(bbsno) as cnt FROM bbs WHERE bbsno=2 AND passwd='1234';
...
7. 글 삭제(D:Delete)
-- 부모글인지 확인
-- 결과값이 0 이상이면 부모글 / 0 이면 부모글 아님
select count(refno) from bbs where refno = 44;
